...
vi.mock('devextreme-react/html-editor', () => {
  function HtmlEditor({ children }: { children?: ReactNode }) { return <div>{children}</div>; }
  function Toolbar({ children }: { children?: ReactNode }) { return <div>{children}</div>; }
  function Item({ name, options }: { name?: string; options?: { text?: string } }) {
    return <button type="button">{options?.text ?? name ?? 'item'}</button>;
  }
